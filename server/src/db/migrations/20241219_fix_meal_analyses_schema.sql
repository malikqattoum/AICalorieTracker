...
-- Add foreign key constraint for meal_id if it doesn't exist
...
SET @constraint_exists = (
    ...
    AND CONSTRAINT_NAME = 'fk_meal_analyses_meal_id'
);
...
SET @user_constraint_exists = (
    SELECT COUNT(*)
    FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'meal_analyses'
    AND CONSTRAINT_NAME = 'fk_meal_analyses_user_id'
);

SET @user_sql = IF(@user_constraint_exists = 0,
    ...
    'SELECT "User foreign key constraint already exists" as message'
);
